...
| **调整参数与官方一致** | **大小（32–512px）/ 位置右·下（0–10000px）/ 自定义名称（≤20 字符）/ 显示隐藏 / 启用**——与官方 dsh-pet 的显示参数同名同界；卡片内 −/+ 调整，悬浮宠可**拖拽**移动（自动持久化位置） |
| 可替换形象 | 宠物 = `assets/pets/<名字>/pet.json` + `emotes/` 文件夹；改文件即换形象，加目录即加新宠物（自动出现在切换菜单） |
| 配置持久化 | 活动宠物 / 启用 / 可见性 / 大小 / 位置 / 名称存于 `~/.config/dsh-web-pets.json` |
