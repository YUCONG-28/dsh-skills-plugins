...
| 会话联动 | `step/start` → thinking；`tool/call` → running + **按工具类型的气泡**；`turn/end` 完成 → success + 「🎉 完成！」；中止/失败/会话销毁 → idle |
| 工具信息适配 | 运行中真实出现的 `bash`、`read`、`write`、`glob`、`grep`、`web_search`、`ssh_*` 等工具，气泡按类型展示**命令 / 文件路径 / 模式 / 查询词**等关键信息（`🖥 命令 · ls -la`、`📖 读取 · src/index.ts`、`🔎 匹配 · **/*.ts`、`🌐 搜索 · …`），工具名经 `state.tool` 可见；未知工具回退「🛠 工具名 · 参数预览」 |
| 点击互动 | 点击桌宠 → 「❤️ 摸头 +1」气泡 |
| 右键菜单 | 切换宠物 / 缩小 / 放大 / **水平·垂直镜像** / 隐藏宠物 |
| **调整参数与官方一致** | **大小（32–512px）/ 位置右·下（0–10000px）/ 自定义名称（≤20 字符）/ 水平·垂直镜像 / 显示隐藏 / 启用**——与官方 dsh-pet 的显示参数同名同界；卡片内 −/+ 调整，悬浮宠可**拖拽**移动（自动持久化位置） |
| 可替换形象 | 宠物 = `assets/pets/<名字>/pet.json` + `emotes/` 文件夹；改文件即换形象，加目录即加新宠物（自动出现在切换菜单） |
| 配置持久化 | 活动宠物 / 启用 / 可见性 / 大小 / 位置 / 名称 / 镜像存于 `~/.config/dsh-web-pets.json` |
...
- **调整参数与官方 dsh-pet 一致**（同名同界）：
  - **大小**：− / ＋ 调节（32–512px）；
  - **位置**：右 / 下 − / ＋ 调节（0–10000px）；
  - **名称**：输入自定义名（≤20 字符）保存；
  - **镜像**：↔ 水平 / ↕ 垂直 切换按钮（与原生 desktop-pets 的 mirror_x/mirror_y 语义一致）；
  - **显示 / 隐藏**：开关我们的宠物（隐藏后右下角保留 🐾 召唤按钮）；
- 底部状态行：当前显示哪只宠物 + 表情状态/气泡（运行中会显示如「🖥 命令 · ls -la」的工具气泡）。
...
- **拖拽移动**：按住桌宠拖动即可换位置（与官方 dsh-pet 一致，松手自动持久化 right/bottom）。
- **右键桌宠** → 菜单：切换宠物（内置 demo / remiel）、缩小 / 放大、**↔ 水平镜像 / ↕ 垂直镜像**、隐藏宠物。
- **隐藏后**：右下角出现 🐾 召唤按钮，点击召回。
- **点击桌宠**：摸头互动气泡。
